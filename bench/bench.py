...
def _bootstrap(addr):
    ...
    for cmd in [
            ...
            'cd go/src/github.com/tfbrother/nsq/apps/nsqd && \
                GOPATH=/home/ubuntu/go /usr/local/go/bin/go build',
            'cd go/src/github.com/tfbrother/nsq/bench/bench_writer && \
                GOPATH=/home/ubuntu/go /usr/local/go/bin/go build',
            'cd go/src/github.com/tfbrother/nsq/bench/bench_reader && \
                GOPATH=/home/ubuntu/go /usr/local/go/bin/go build',
    ...
    for id, addr in nsqd_hosts:
        try:
            ...
            for cmd in [
                    ...
                    'GOMAXPROCS=32 ./go/src/github.com/tfbrother/nsq/apps/nsqd/nsqd \
    ...
    for id, addr in producer_hosts:
        for nsqd_tcp_addr in nsqd_tcp_addrs:
            ...
            try:
                ...
                for cmd in [
                        'GOMAXPROCS=2 \
                            ./go/src/github.com/tfbrother/nsq/bench/bench_writer/bench_writer \
        ...
        for id, addr in consumer_hosts:
            for nsqd_tcp_addr in nsqd_tcp_addrs:
                ...
                try:
                    ...
                    for cmd in [
                            'GOMAXPROCS=8 \
                                ./go/src/github.com/tfbrother/nsq/bench/bench_reader/bench_reader \
                                --topic=%s --nsqd-tcp-address=%s:4150 --deadline=\'%s\' --size=%d \
                                --rdy=%d' % (
                                topic, nsqd_tcp_addr, deadline.strftime('%Y-%m-%d %H:%M:%S'),
